...
impl Script {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        if bytes.len() < length_consumed + length.value as usize {
            return Err(BitcoinError::InsufficientBytes);
        }

        let script_bytes = bytes[length_consumed..length_consumed + length.value as usize].to_vec();
        let total_consumed = length_consumed + length.value as usize;
